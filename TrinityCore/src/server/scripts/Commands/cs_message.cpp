...
public:
    ...
    // MARK: global announce
    // TODO: Implement SlackHook
    ...
    // TODO: Implement Slack callback (for recieving messages FROM slack
    static bool HandleAnnounceCommand(char const* uname, char const* args)
    {
        if (!*args)
            return false;
        
        std::string name("Console");
        // Commented: Get name from ChatHandler - RE: Name from Slack = Username
        
        //if (WorldSession* session = handler->GetSession())
          //  name = session->GetPlayer()->GetName();
        
        //Post GMtext (Announcement)
        sWorld->SendGMText(LANG_GM_ANNOUNCE_COLOR, uname, args);
        // DO NOT Update Slack
        return true;
    }
